  ...
  try {
    ...
    if (response.message === 'success' && response.data) {
      if (response.data.length > 0) {
        console.log('🏦 Time data received:', response.data.length, 'points')
        ...
        console.log('🏦 No new time data available')
      }
    } else {
      error.value = response.error || 'Failed to load time data'
...
function initChart() {
  ...
  console.log('🏦 Initializing bank chart')
  ...
  chart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: `Banking Metrics - ${props.selectedCountry}`,
          font: {
            size: 14
          }
        },
      ...
      animation: {
        duration: 0 // No animation for performance
      }
    }
  })
...
function startAutoUpdate() {
  ...
  updateInterval = setInterval(() => {
    loadData()
  }, 2000) // Update every 2 seconds like legacy
...
watch(() => props.selectedCountry, async (newCountry) => {
  if (newCountry) {
    ...
    // Reset data for new country
    dataPoints.value = []
    lastTimestamp.value = 0
    
    // Update chart title
    if (chart) {
      chart.options.plugins!.title!.text = `Banking Metrics - ${newCountry}`
    }
